...
var newPlayerJoin = document.querySelector(".newPlayerJoin");
...
const init = () => {
    ...
    gltfLoader.load(
      ...
      (xhr) => {
        ...
        if (total === 100) {
          ...
          water.rotation.z = 11;
          water.position.set(-3.5, 1.9, -13.8);
          water.scale.set(52.2, 48.7, 1);
          water.name = "water";
          cityGroup.add(water);
        }
      }
  ...
  window.addEventListener("resize", function () {
    resizeM(isMobile, playersPeer, renderer, labelRenderer, socketName);
  });
  // console.clear();
};
...
const animate = () => {
  ...
  if (playersPeer[socketName]) {
    ...
    smallAvtar.style.left = 110 + x + "px";
  ...
  if (playersPeer[socketName]) {
    ...
    if (objectArray && objectArray.length) {
      ...
      if (downIntersectedObjects && downIntersectedObjects.length) {
        if (downIntersectedObjects[0].object.name == "Base") {
          playersPeer[socketName].children[0].position.copy(
            characterMeshPosition
          );
        } else {
